...
export function JobsPanel() {
  ...
  const load = useCallback(() => {
    return fetch(`/api/jobs?limit=25${visaOnly ? "&visa=1" : ""}`)
      .then((res) => res.json())
      .then((data) => {
        setItems(data.items ?? []);
    ...
    <section className="glass mt-6 p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={refresh} disabled={ingesting} className="glass-btn glass-btn-primary">
        ...
        {items.map((job) => (
          <li key={job._id} className="flex items-start justify-between gap-3 py-3">
            ...
            <div className="flex shrink-0 items-center gap-1.5">
              ...
              <span className="glass-chip">{job.source}</span>
            </div>
          </li>
        ))}
