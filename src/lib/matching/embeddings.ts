...
import { embedProvider, type EmbedProvider } from "@/lib/ai";
...
export async function embedProfile(
  userId: ObjectId,
  embed: EmbedProvider = embedProvider(),
): Promise<number[] | null> {
  ...
  const [vector] = await embed.embed([profileToText(profile)]);
...
export async function embedMissingJobs(
  limit = 200,
  embed: EmbedProvider = embedProvider(),
): Promise<number> {
  ...
  const vectors = await embedInBatches(missing.map((j) => jobToText(j)), embed);
  await Promise.all(
    missing.map((j, i) =>
